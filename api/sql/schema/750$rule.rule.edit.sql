    ...
    BEGIN TRANSACTION
        ...
        (
            SELECT records.x.value('(./splitAssignmentId)[1]', 'int') AS splitAssignmentId,
                ...
                records.x.value('(./debitTenantId/text())[1]', 'bigint') AS debitTenantId,
                records.x.value('(./creditTenantId/text())[1]', 'bigint') AS creditTenantId
        ...
        SET debit = r.debit,
            ...
            debitTenantId = r.debitTenantId,
            creditTenantId = r.creditTenantId
        WHEN NOT MATCHED BY TARGET THEN
        INSERT (splitNameId, debit, credit, minValue, maxValue, [percent], description, debitTenantId, creditTenantId)
        VALUES (r.splitNameId, r.debit, r.credit, r.minValue, r.maxValue, r.[percent], r.description, debitTenantId, creditTenantId)
